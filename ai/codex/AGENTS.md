...
- Delegate simple, quick, straightforward, low-risk tasks to the `fast_worker` agent, including search, documentation, and test tasks.
- Use `worker` when task complexity is uncertain or when correctness benefits from deeper reasoning; reserve `fast_worker` for clearly bounded, easy work.
