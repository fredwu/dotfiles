...
3. Use explicitly supplied candidate paths exactly, including prior definitive outputs. Otherwise inspect `.local/` and infer one coherent candidate set from the user's selector, or from shared task and plan content when no selector is given.
