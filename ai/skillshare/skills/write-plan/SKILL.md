...
Review requirement coverage, evidence, dependencies, validation, and scope. An independent challenge is optional when useful. Read back the saved plan, remove contradictions and duplication, then link it and report material verification limits. In the chat response, explicitly call out high-risk recommendations, their rationale, mitigations, residual risks, and any required execution authorization or essential missing input. Keep these disclosures concise; do not turn them into routine approval requests. Do not execute the plan. Keep working state in context; create no scratch files, separate reports, ledgers, or supporting artifacts under this plan-only write contract. The saved plan is a durable deliverable, including when stored in `.local/`; preserve it after completion. Include agent-purpose-only temporary-file creation and removal after use in execution steps that need scratch, with exact task ownership and no deletion of unrelated files or durable deliverables.
