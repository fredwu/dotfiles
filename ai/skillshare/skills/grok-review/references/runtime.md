...
The caller owns cleanup outside the child process; an `exec` replacement cannot perform post-exit cleanup. Create scratch only for agent isolation, request transfer, completion checks, or diagnosis, never merely for user presentation. Track exact task-created paths, including requests, outputs, copied schemas, logs, probe sentinels, empty working directories, and isolated environment roots. Wait for consumers to exit and finish assessment, then remove those paths and verify removal on success, preflight failure, call failure, or abandonment. Stop task-owned processes before removing files they still use. Preserve canonical schemas, normal pre-existing CLI state, requested durable deliverables, and unrelated files; never delete a shared directory wholesale. Always remove the transient environment after each attempt; it is not eligible for handoff retention. Retain only non-secret request or diagnostic scratch needed for active agent continuation or handoff, with an explicit cleanup owner and removal point; that owner must remove it after use. Report active handoff paths or cleanup failures, not raw logs.
