...
5. If the brief says there is no material difference, leave ordinary plan prose unchanged unless the user supplied additional revision instructions or the whole-plan requirement audit below finds a material defect. Still remove any existing `## Requirement updates (verbatim)` section as required below. Do not create other editorial churn simply to demonstrate an edit.
