---
name: revise-plan
description: Revise the current saved analysis plan in place using the latest completed compare-plans findings and any additional user guidance, preserving only the originating write-plan requirement as verbatim requirement history and integrating later guidance throughout ordinary plan prose instead of recording update history or settled decisions. Use only after compare-plans when the user explicitly invokes `/revise-plan` or `$revise-plan`, optionally referring to finding numbers or adding revision guidance.
---
...
2. Interpret content following `/revise-plan` or `$revise-plan` as optional working guidance. It may identify the current plan, select findings by `#`, override a recommendation as a user preference, clarify the desired outcome, or request another evidence-backed adjustment. Apply authoritative changes throughout the plan where relevant, but do not quote this content into the saved plan as requirement history.
