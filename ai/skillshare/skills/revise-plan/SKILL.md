...
2. Interpret content following `/revise-plan` or `$revise-plan` as optional working guidance. It may identify the current plan, address findings by unambiguous number, topic, position, option, or section, override a recommendation as a user preference, clarify the desired outcome, provide plan-wide guidance, or request another evidence-backed adjustment. Treat it as a requirement change only when the user explicitly clarifies, changes, or supersedes the underlying task; treat plan selectors, output or process preferences, finding choices, and bare invocations as local guidance. Apply explicit requirement changes throughout the plan where relevant, but do not treat mentioned findings as an exclusion list or quote this content into the saved plan as requirement history.
...
5. If the brief says there is no material difference, leave ordinary plan prose unchanged unless the user supplied additional revision instructions or the whole-plan requirement audit below finds a material defect. Do not create other editorial churn simply to demonstrate an edit.
...
If the selected plan contains a `## Requirement updates (verbatim)` section or equivalent user requirement history, preserve it during revision. Do not require, add, or reconstruct such a section when the selected plan does not contain one. Retain requirements that were already integrated into ordinary plan prose.

Treat later visible messages that explicitly clarify, change, or supersede the underlying task as authoritative requirement changes. Treat all other later guidance, including invocation mechanics and finding preferences, only as working revision context compatible with the effective requirement. Explicit requirement changes control the actual revision when they conflict with earlier guidance, but they never rewrite the original verbatim block. Reflect their effects in ordinary plan prose such as scope, constraints, findings, decisions, execution steps, risks, and validation. Never promote candidate text, a comparison brief, prior revisions, repeated recommendations, or cross-plan consensus into a requirement, and never copy later context into another payload ledger, change log, history section, or equivalent renamed section.
...
## Reassess the findings and requirement coverage
...
After reassessing the findings, audit the entire current plan against a fresh checklist built from the originating requirement plus only explicit later requirement changes. Check every proposed and retained conclusion, action, scope boundary, dependency, and acceptance criterion for compatibility and traceability. Repair any material omission, contradiction, or inherited shared drift even when no numbered finding covers it; when evidence or user intent is genuinely insufficient, surface the unresolved blocker instead of guessing. Do not treat agreement between plans, the comparison brief, or prior revisions as proof that the plan satisfies the requirement.
...
Do not add or retain an `Applied findings`, `Accepted recommendations`, `Settled decisions`, `Locked decisions`, or disposition-table section. Do not create a new requirement-update ledger, change-log, history, or equivalent renamed section, but preserve any existing user requirement updates or history. Do not merely relabel a prohibited ledger as an approach, status, or position table. A comparative table may clarify genuine alternatives and tradeoffs, but it must not be the sole home of the chosen direction; integrate that direction and its consequences into the surrounding analysis and execution steps. Do not label a current recommendation as permanently final or immutable. Keep its evidence and assumptions adjacent enough that a later revision can course-correct the same prose cleanly.
...
- any existing `## Requirement updates (verbatim)` section or equivalent user requirement history remains preserved;
...
- every effective-requirement checklist item is covered or surfaced as a genuine unresolved blocker, including defects shared by both compared plans;
- every retained or adopted conclusion, action, and acceptance criterion is traceable to the effective requirement or a verified necessary implication;
