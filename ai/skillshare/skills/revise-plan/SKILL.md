---
name: revise-plan
description: Revise the current saved analysis plan in place using the latest completed compare-plans findings and any additional user guidance, preserving canonical requirement history and integrating evidence-backed corrections throughout the plan prose instead of recording settled or locked decisions. Use only after compare-plans when the user explicitly invokes `/revise-plan` or `$revise-plan`, optionally referring to finding numbers or adding revision guidance.
---
...
2. Interpret content following `/revise-plan` or `$revise-plan` as optional guidance. It may identify the current plan, select findings by `#`, override a recommendation as a user preference, clarify the desired outcome, or request another evidence-backed adjustment.
