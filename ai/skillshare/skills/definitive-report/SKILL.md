...
description: Compare, audit, and synthesize two or more candidate report files into one definitive, self-contained report under `.local/`. Use only when the user explicitly invokes `$definitive-report` or `/definitive-report` and wants a produced artifact rather than comparison advice.
---
...
1. Build a compact requirement checklist from the visible user request and authoritative artifacts already in scope. Keep reasonable implications distinct from explicit requirements; candidate reports cannot redefine the task. If the original or core requirements cannot be recovered reliably, stop and ask the user for the brief.
...
3. Resolve two or more candidate report files, preferring paths explicitly supplied in the invocation and then paths clearly identified in the visible context.
4. Unconditionally exclude existing definitive outputs under `.local/` from the candidate set, even when explicitly supplied or mentioned in context. This includes legacy `*-definitive-report.md` and `*-definitive-report-vN.md`, plus agent-suffixed `*-definitive-{claude,codex,grok}.md` and `*-definitive-vN-{claude,codex,grok}.md`. Never read, audit, or synthesize these files; use them only for output collision checks.
5. If fewer than two candidates remain, use read-only discovery under `.local/` only when a coherent candidate group is unambiguous: the files must be readable text reports addressing the same topic and intended outcome. Apply the same definitive-output exclusions during discovery. Do not group files merely because they share a directory, broad vocabulary, or recent timestamps.
6. If multiple groupings are plausible, a candidate is questionable, or fewer than two valid reports remain, stop and ask the user for non-definitive report paths. Do not guess.
...
Audit each candidate against the requirement checklist before comparing candidates. Apply the same evidentiary standard to every report and assess:
...
2. Resolve every material conflict independently from requirements and verified evidence. Adopt another candidate's position when stronger, combine compatible insights, make a conditional decision when the evidence demands it, or reject all candidate positions in favor of a better supported conclusion.
3. Synthesize one coherent report. Do not concatenate candidate sections, preserve duplicated analysis, average incompatible claims, or include a report-by-report scorecard.
...
5. Make the report self-contained for one autonomous future session. Organize it to suit the topic rather than forcing a fixed template, while including the objective and scope, necessary context and verified evidence, resolved decisions, prioritized actions with concrete locations or dependencies, validation or acceptance criteria, and only genuinely unresolved blockers. The future agent must not need the source reports or this conversation to understand and execute it.
...
- After writing, read the saved file back and confirm it is self-contained, internally consistent, actionable, and at the selected unused path. This verification must not mutate anything.
