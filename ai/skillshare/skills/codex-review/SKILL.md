...
Include `code-review`'s lenses, evidence threshold, priority semantics, and finding fields, plus the canonical round and permitted focus context when called by a loop. Apply all lenses in standalone and broad loop rounds; in focused loop rounds, use the canonical discovery priorities and repair coverage. Require inspection of every content block, preserved scope, repository-relative `path:line` evidence, and disclosure of unfinished work. Do not read host paths outside the snapshot. Follow applicable agent routing: workers receive relevant snapshot content, inherit the read-only sandbox and empty tool environment, and finish before synthesis. Forbid nested review skills and additional top-level Codex processes.

Require one terminal JSON object matching [the Codex output schema](references/output.schema.json). Emit it only after complete inspection or demonstrated inability to inspect; never as progress. After the call, enforce [the external schema](../code-review/references/external-review-result.schema.json) cardinality: `findings` when there are findings, empty `findings` for `clean` or `incomplete`. Omit caller-only assessment fields. `clean` requires complete inspection.
...
For embedded reviews or requested JSON, return one canonical `REVIEW_RESULT`. Otherwise give accepted and partial findings, declined count, inspected surface, and material limits in plain language. Skip stock headings, repeated summaries, and raw logs. Save a user report only if requested. Never commit, push, publish comments, or make remote writes without separate authorization.
