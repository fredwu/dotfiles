...
For embedded reviews or requested JSON, return one canonical `REVIEW_RESULT`. Otherwise summarize accepted and partial findings, declined count, inspected surface, and residual risk. Do not duplicate the result or save a report unless useful or requested; omit raw logs. Remove only validated task-created temporary paths. Keep non-secret diagnostics only when useful; report any retained paths. Never commit, push, publish comments, or make remote writes without separate authorization.
