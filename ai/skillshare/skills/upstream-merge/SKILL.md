...
6. Compare the result with both parents. Check for conflict markers, dropped behavior, accidental reversions, and unrelated changes. Deliberately inspect the merge-changed and directly affected task surface for confirmed legacy, redundant, duplicate, dead or unused, obsolete, superseded, and no-longer-needed compatibility code exposed or introduced by the merge. Remove it only when safe and authorized while preserving required behavior, explicitly required compatibility, intentional project customization, unrelated work, and scope. A clean result is acceptable; do not invent work. Run project-standard formatting, static analysis, tests, and builds for every affected area.
